...
namespace Core.Mappy.Configuration
{
    public class MapperConfiguration<TSource, TDestination>
    {
        ...
        private string GetMemberName<TProperty>(Expression<Func<TDestination, TProperty>> expression)
        {
            if (expression.Body is MemberExpression memberExpression)
            {
                return memberExpression.Member.Name;
            }
            throw new ArgumentException("The destination member must be a property or field access.", nameof(expression));
        }
    }
}
